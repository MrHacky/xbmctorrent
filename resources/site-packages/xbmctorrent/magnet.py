...
def play(uri):
    ...
    if uri.startswith("magnet:"):
        #host = "t60p.local"
        host = "pi2-gentoo.local"
	#if host.endswith(".local"):
        #    host = subprocess.check_output(["/storage/avahi-resolve-host-name", host]).split("\n")[0].split("\t")[1]
        item = {
            #"path": "http://" + host + ":8081/playlist/" + uri
            #"path": "http://" + host + ":8081/start/redir.playlist/" + uri
            "path": "http://" + host + ":3000/start/redir.playlist/" + uri
        }
        #plugin.set_resolved_url(item)

        plugin.set_resolved_url({ "path": "file:///storage/videos/loadingbar.mp4"})
	xbmc.Player().play(item["path"])
